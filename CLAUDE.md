...
- Единственный транспорт — `scripts/ask.sh`, второго не заводим.
- Скилл владеет только промптом и форматом отчёта, ничем больше.
- PR со вторым раннер-скриптом отклоняется: доучиваем `ask.sh`, а не пишем рядом второй.
...
README пишет только Серёжа. Нужна правка — предложи ему текст словами, вносит он.
Переводы README.ru/README.zh синхронизируются с его английским текстом — это можно агентом.
